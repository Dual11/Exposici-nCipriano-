...
public class DemoData {
    ...
    CommandLineRunner initData(AutorRepository autorRepo,
        ...
        return args -> {
            Autor a1 = autorRepo.save(Autor.builder().nombre("Isaac Asimov").pais("Rusia/EEUU").build());
            Autor a2 = autorRepo.save(Autor.builder().nombre("Ursula K. Le Guin").pais("EEUU").build());

            Categoria c1 = catRepo.save(Categoria.builder().nombre("Ciencia Ficción").descripcion("Sci-Fi").build());
            Categoria c2 = catRepo.save(Categoria.builder().nombre("Fantasía").descripcion("Fantasy").build());

            Libro l1 = libroRepo.save(Libro.builder().titulo("Fundación").isbn("978-0-553-80371-0").anioPublicacion(1951).autor(a1).categoria(c1).build());
